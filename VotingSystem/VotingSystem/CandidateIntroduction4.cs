...
{
    public partial class CandidateIntroduction4 : Form
    {
        public CandidateIntroduction4()
        {
            InitializeComponent();
        ...
        SqlConnection mycon;
        SqlCommand command;
        public CandidateIntroduction4(Image image)
        {
            InitializeComponent();
        ...
        private void CandidateIntroduction4_Load(object sender, EventArgs e)
        {
            SqlConnection conn = new SqlConnection(@"Data Source=DESKTOP-6UGITVT;Initial Catalog=Voting;Integrated Security=True");

            conn.Open();
            SqlCommand cmd = new SqlCommand("select Image from CandidateImage where CandidateImgId='4'", conn);
