...
{
    public partial class CandidateIntroduction3 : Form
    {
        public CandidateIntroduction3()
        {
            InitializeComponent();
        ...
        SqlConnection mycon;
        SqlCommand command;
        public CandidateIntroduction3(Image image)
        {
            InitializeComponent();
        ...
        private void CandidateIntroduction3_Load(object sender, EventArgs e)
        {
            SqlConnection conn = new SqlConnection(@"Data Source=DESKTOP-6UGITVT;Initial Catalog=Voting;Integrated Security=True");

            conn.Open();
            SqlCommand cmd = new SqlCommand("select Image from CandidateImage where CandidateImgId='3'", conn);
