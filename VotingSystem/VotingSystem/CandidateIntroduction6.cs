...
{
    public partial class CandidateIntroduction6 : Form
    {
        public CandidateIntroduction6()
        {
            InitializeComponent();
        ...
        SqlConnection mycon;
        SqlCommand command;
        public CandidateIntroduction6(Image image)
        {
            InitializeComponent();
        ...
        private void CandidateIntroduction6_Load(object sender, EventArgs e)
        {
            SqlConnection conn = new SqlConnection(@"Data Source=DESKTOP-6UGITVT;Initial Catalog=Voting;Integrated Security=True");

            conn.Open();
            SqlCommand cmd = new SqlCommand("select Image from CandidateImage where CandidateImgId='6'", conn);
