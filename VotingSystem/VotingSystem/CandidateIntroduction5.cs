...
{
    public partial class CandidateIntroduction5 : Form
    {
        public CandidateIntroduction5()
        {
            InitializeComponent();
        ...
        SqlConnection mycon;
        SqlCommand command;
        public CandidateIntroduction5(Image image)
        {
            InitializeComponent();
            pictureBox1.Image = image;
        }
        private void CandidateIntroduction5_Load(object sender, EventArgs e)
        {
            SqlConnection conn = new SqlConnection(@"Data Source=DESKTOP-6UGITVT;Initial Catalog=Voting;Integrated Security=True");

            conn.Open();
            SqlCommand cmd = new SqlCommand("select Image from CandidateImage where CandidateImgId='5'", conn);
